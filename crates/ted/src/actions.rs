...
//!
//! `ctrl-p` is `file_finder::Toggle`, `ctrl-tab` is `tab_switcher::Toggle`,
//! `space f` is a two-keystroke vim binding for the first of those, and every
//! one of them opens a `Picker` modal `ted` cannot paint. Rather than duplicate
//! the keymap to find those keystrokes — which is exactly the drift SPEC §13.1
//! is trying to avoid — `ted` rewrites the *binding*: GPUI still resolves the
//! keystroke, and only the action at the end of it changes, so multi-keystroke
//! bindings and anything in the user's `keymap.json` keep working.
//!
...
impl Surface {
    pub fn action(self) -> Box<dyn Action> {
        match self {
            Self::Finder => OpenFileFinder.boxed_clone(),
...
pub fn surface_for(action: &dyn Action) -> Option<Surface> {
    ...
    if action.is::<OpenFileFinder>() || action.is::<workspace::ToggleFileFinder>() {
        Some(Surface::Finder)
    } else if action.is::<OpenBufferSwitcher>()
...
pub const REPLACED_NAMESPACES: [&str; 3] = ["file_finder", "go_to_line", "tab_switcher"];
...
pub fn init(cx: &mut App) {
    ...
    cx.on_action(|_: &OpenFileFinder, cx| request(Surface::Finder, cx));
...
mod tests {
    ...
    fn every_modal_ted_replaced_resolves_to_the_surface_that_replaced_it() {
        let cases: [(Box<dyn Action>, Surface); 8] = [
            (
                workspace::ToggleFileFinder::default().boxed_clone(),
                Surface::Finder,
            ),
            (OpenFileFinder.boxed_clone(), Surface::Finder),
            (
