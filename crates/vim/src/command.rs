        ...
        if let Some(range) = &action.range {
            vim.update_editor(cx, |vim, editor, cx| {
                ...
                let filename = if filename.is_empty() {
                    let Some(filename ) = editor.buffer().read(cx).
                         as_singleton().and_then(|buffer|
                            buffer.read(cx).file()
                        ) else {
                            return;
                        };
                    filename.path().display(util::paths::PathStyle::Posix).to_string()
                ...
                editor.project().unwrap().update(cx, |project, cx| {
                    ...
                    worktree.update(cx, |worktree, cx| {
                        let path = RelPath::unix(&filename).unwrap_or(RelPath::empty());
                        ...
                        cx.spawn_in(window, async move |this, cx| {
                            ...
                            let _ = this.update_in(cx, |worktree, window, cx| {
                                let path = RelPath::unix(&filename).unwrap_or(RelPath::empty());
                                worktree
                                    .write_file(path.into(), text.clone(), line_ending, cx)
...
mod test {
    ...
    #[gpui::test]
    async fn test_range_w_command(cx: &mut TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
